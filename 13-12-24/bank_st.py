...
class Bank:
  ...
  def deposit(self):
      ...
      amount = st.number_input("Enter amount to deposit:", key="deposit_amount")
  ...
  def withdraw(self):
      ...
      amount = st.number_input("Enter amount to withdraw:", key="withdraw_amount")
      min_bal = 500
  ...
  def validate(self):
      chance = 1
      while chance > 0:
          pin = st.number_input("Enter your pin:", key=f"pin_input_{chance}",min_value=0)  # Unique key for each pin entry
          actual_pin = 1234
          if pin == actual_pin:
              obj.viewoptions()
              break
